...
// 断面上の点は極座標 (r, a) で表す（a は断面の上側 +y から測った角度）。
// 曲げ応力に出てくる「中立軸からの高さ」は r cos(a) であり、上の y 軸とは別物なので
// 式でも y を使わず r cos a と書く。
//
// 荷重（すべて棒全長にわたって一定）:
//   P : 軸方向の荷重 [kN]（引張が正。コード中のキーは N）
//   M : 両端に加わる曲げモーメント [N·m]（z 軸まわり = 純曲げ）
//   T : ねじりモーメント [N·m]（x 軸まわり、右ねじが正）
//
// 応力（単位はすべて MPa = N/mm^2）:
//   σx  = P/A - M·r·cos(a)/I
//     符号がマイナスなのは図の曲げモーメントの向きに合わせるため。図では棒の +x 端に
//     +M（z 軸まわり、右ねじ）の偶力を描いており、断面の釣り合い M_ext,z = -∫y·σx dA
//     から σx = -M·y/I になる（M>0 なら下側が引張＝サギング）。
//   τxy = T·r/Ip
//   σy = σr = τyr = τrx = 0
