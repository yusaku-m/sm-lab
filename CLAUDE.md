...
#### 画面構成（スマホで応力円とコンター図を1画面に収める）

カードは5枚: `rod-card` / `mohr-card`（円だけ） / `detail-card`（φスライダー・応力要素・成分表） /
`controls-card` / `notes-card`。**`mohr-card`と`detail-card`を分けてあるのは、スマホで
「モールの応力円」と「丸棒のコンター図」を続けて並べて1画面に収めるため**（φスライダーや表が
間に入ると入りきらない）。PCは2列（`grid-template-areas`の4行）、スマホは1列で
`mohr → rod → detail → ctrl → notes` の順。

1画面に収めるための具体策（`@media (max-width: 760px)`、**style.cssの末尾に置いてある**）:
- kicker と `.sub`（説明文）を非表示、カードのpaddingと見出しを縮める
- 2つの図をそれぞれ `30svh`（`svh`＝アドレスバーを除いた高さ。`vh`のフォールバック付き）
- 平面のチェックボックスは短縮表記に切り替える（`.long`/`.short`をCSSで出し分け）
- コンター選択のツールバーを1行に固定（`nowrap`＋selectを可変幅に、ラベルは隠す）
- 図の下の説明（`.hint`）は後半を隠す（`.hint-more`）
...
`.viewport` の `getBoundingClientRect().bottom <= window.innerHeight` を見ればよい。
...
- **「応力円が最大の点へ」**（`app.js`の`maxCirclePoint()`）… 3つの円のうち最大の直径
  σ1−σ3（= 2τmax）が最大になる点を探して探触点にする。この応力状態では解析的に必ず
...
探触点の三面体（e_x / e_θ / e_r の矢印）の先に `x` `θ` `r` のラベルを出している。実装は
