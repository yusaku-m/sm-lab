...
- **応力要素の図と成分表（`.element-wrap`）は非表示**。回転インターフェース（`.rotate-block`）だけ残す
- 2つの図をそれぞれ `23svh`（`svh`＝アドレスバーを除いた高さ。`vh`のフォールバック付き）
...
- **「単純引張 / 単純曲げ / 単純ねじり」**（`app.js`の`PRESETS`）… 指定の荷重以外を 0 にする。
  ...
  荷重パネル（3つ全部）と丸棒パネル（`#presets-rod`、引張とねじりの2つだけ・スマホのみ表示）の
  両方に置いてある。スマホでは荷重パネルが画面外なので手元にも要るという理由。
