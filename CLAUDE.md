...
sfd-bmd/            # 「SFD・BMD」単元（equilibrium/ の model.js と style.css をそのまま読む）
  index.html         # ../equilibrium/style.css ＋ style.css（この単元だけの追加）
  model.js           # 支点→反力（equilibrium の analyze）、断面力 V・M、区間ごとの式、グラフの点列
  symbolic.js        # 荷重の記号・L・x の多項式。反力を記号で解き、V・M を x の式にする（グラフのラベル用）
  figure.js          # 梁・支点・荷重・仮想断面の SVG と V・M のグラフ（BeamFigure / DiagramPlot）
  app.js             # 状態・支点と荷重の一覧・式・断面のアニメーション・URLハッシュ/QR
```
...
- 位置 `t` は長さを 1 とした無次元量（左端 0）。力は kN、モーメントは kN·m（`Lm`=長さ[m] を掛ける）
- 力の向き `dir` は +x から反時計まわり [deg]（上向き 90、下向き −90）。
  UI・式では「上向き/下向き」＋「鉛直からの傾き θ（右へ倒すと正）」に分解して見せる
  （`decompose()`/`compose()`）。**資料（week1.py）が鉛直からの角度＋cos で書いているのに合わせた**
- **角度は度数（30° など）で統一**。吸着した角も π/6 ではなく 30° と書く（2026-09-25、ユーザーの指示。
  ユーザーは「弧度法に統一」と書いたが、「吸いついたときはラジアン、他では弧度法」と
  ラジアンと対比していたので度数の意味と解釈した）。cos 30° = √3/2 のような厳密値は残している
- モーメントは反時計まわり正。式は「O からの距離 × P cos θ」を出して回す向きで符号を決める書き方。
- **長さの記号は L / x の切り替え**（`lengthSym()`）。x は「切り取った自由体の長さ」の意味で、
  書き方は L と全く同じ（O は x/3、腕は 5x/12 など）。当初は x を座標とみなして
  `±P(x_i − x_O)` の形にしていたが、ユーザーの指示（2026-09-25）で L と同形式に揃えた
- 各力のモーメントは `M_O(P)`（「P による O まわりのモーメント」）と書く。`M_1, M_2` は
  集中モーメントの記号 `M_1` と衝突するので使わない
- 厳密な係数は `(n/d)·√s` で持つ（`makeCoef`/`mulCoef`/`divCoef`/`coefTex`）。分数に直せない位置
  （吸着しなかったとき）は小数に落ちる。吸着しなかった位置は 1/200 刻みに丸めてある

#### 記号（ユーザーの指示、2026-09-25）

| 種類 | 1 つだけ | 複数 | 図 |
|---|---|---|---|
| 集中荷重 `point` | `P`, `θ` | `P_1`, `θ_1` … | 実線の矢印（長さ ∝ 大きさ） |
| 未知反力 `reaction` | `R`, `θ_R` | `R_1`, `θ_{R_1}` … | 破線の矢印（長さ一定、仮定の向き） |
| 集中モーメント `moment` | `M` | `M_1` … | 実線の円弧（30°/kN·m） |
| 反力モーメント `rmoment` | `M_R` | `M_{R1}` …（Beam.py と同じ形） | 破線の円弧（230° 一定） |
| 分布荷重 `dist` | `w` | `w_1` … | 棒に向かう矢印の列（高さ ∝ 強さ） |

**その種類が 1 つだけなら添え字なし**（`namesOf()`）。種類ごとに 1 から番号を振る。
荷重の数が変わると記号が変わる（P → P_1）ので、`syncList()` は種類の並びが変わったら一覧を作り直す。

#### 未知量の解き方（`analyze()`）

未知量は「仮定した向きの大きさ u」。各荷重の `effect(a, tO, u)` が `{fx, fy, m}` を返し、
u=1 のときの値を係数にしてつり合いの式（ピン: `M_O = 0` の 1 本、自由体: `F_x, F_y, M_O` の 3 本）を作る。
正規方程式（最小二乗）で解き、状態を `status` に入れる:
- `solved` … 一意に決まり、全部の式を満たす。負なら仮定と逆向き
- `under` … 決まらない（未知量が式より多い、O を通る力なので `M_O` に現れない 等）
- `inconsistent` … どう選んでもつり合わない（例: 斜めの荷重なのに横向きの反力が無い）
未知量が 1 つで `M_O` の式に現れるときは記号のまま解いて見せる（`symbolicSolution()`、
例: `R = \frac{\sqrt3}{4}P`、`R = \frac12 P - \frac{M}{x}`）。2 つ以上は数値だけ。
アニメーションは解いた値を入れて動かす（解ければつり合うので動かない。決まらなければ 0 扱い）。

#### 吸着（ユーザーの要望）

- 位置: 長さの 1/2, 1/3, 1/4, 1/5, 1/10 の倍数（`SNAP_DENOMS`）。許容 10px、分母が小さいほど強く吸う
- 傾き: 0°, 30°, 45°, 60°, 90°（`TILT_SNAPS`）。許容 ±5°、それ以外は 1° 刻み
- 矢印の先端は**まず上下だけ**（縦モード）。先端を作用点から横に 46px 以上引いたら
  そのドラッグ中は斜めモード（`OBLIQUE_DX`）。最初から斜めの荷重は斜めモードで始まる。
  未知反力は長さ一定で向きだけ変わる
- 円弧の先端は 12 時の位置から測った角度を前回の値から連続につないで（±360° の飛びを消して）読む。
  集中モーメントは 30° ごとに 1 kN·m、反力モーメントは向きだけ
- URL の小数（`0.333333`）は `exactFrac()` で分数の位置へ戻す（戻さないと `0.333L` 表記になる）

#### 分布荷重（`dist`、2026-09-25 追加）

- 状態は `{ t1, t2, w [kN/m], shape: 'u'|'r'|'l', up }`（等分布／三角形・右が最大／三角形・左が最大）。
  **位置は `t` ではなく `t1`/`t2`** なので、`a.t` を前提にした処理（一覧の位置欄・寸法線等）では分岐が要る。
  記号 `w` は Grading の `Action.DistributedLoad`（強さ `w_0`）に合わせた
- 式は「合力（分布の面積）に置きかえて図心に作用させる」書き方: 合力 $rac12 w	imesrac12L=rac14wL$ →
  $M_O(w) = -rac14 wL	imesrac13L = -rac1{12}wL^2$。形の係数は `DIST_SHAPES`（合力の係数 `k`・図心 `c`）。
  `lin` の次数 `p` が 2 になるので、`linTex()`/`momentSym()` は次数ごとにくくる（`powUnit()`）
- 図: 下向きは棒の上、上向きは棒の下に描く（荷重の側から棒を押す描き方）。寸法線は区間の長さと、O から図心まで
- **「分布荷重の表示」**（`state.distView`、ツールバーのセレクト。分布荷重があるときだけ出す）:
  `dist`「分布荷重：分布荷重のみ表示」（**既定**）／`res`「分布荷重：等価な集中荷重のみ表示」／
  `both`「分布荷重：分布荷重と等価集中荷重両方表示」（文言はユーザーの指定。選択肢に「分布荷重：」が
  入っているので、セレクトの前の見出しは付けず `aria-label` だけにしている）
  （2026-09-25、ユーザーの要望。既定を「分布荷重のみ」にするのもユーザーの指定）。
  `res` の合力は集中荷重と同じ縮尺の実線の矢印で、先端のドラッグで合力→強さ w を逆算、軸で区間ごと移動。
  区間は図に無いので区間の長さの寸法線も引かない。`both` の合力は破線で、長さは `RESULT_MAX` で頭打ち
  （`resultLen()`）。URL は既定以外のときだけ `dv=res|both` を書く（無ければ既定に戻す）
- 操作: 頭の線の丸（等分布は中央、三角形は最大の側）を上下で強さ（棒の反対側まで引くと向きが反転）、
  両端の四角で区間の端（反対の端を越えたら持ち手を入れ替え）、中の矢印の列で区間ごと移動
  （長さを保ったまま、始点か終点の吸い付くほうに合わせる）
- 記号のラベルは強さの丸より外側に置く。丸に重ねると持ち手が文字に隠れて見えにくく、
  横に置くと端の四角に重なる

**`effect()` の `m` は kN·L 単位**（呼ぶ側で `Lm` を掛ける）。集中モーメント・反力モーメントは kN·m の値を
`Lm` で割って返すこと。2026-09-25 まで割っておらず、**長さが 1000 mm 以外だと合計 $M_O$ と反力モーメントの解が
L 倍ずれていた**（分布荷重の検証で L=2000 mm にして発覚。L=1000 mm だと Lm=1 で表に出ない）。

#### 荷重の種類を足すとき

`model.js` の `ACTION_TYPES` に `names(i)` / `create(t)` / `effect(a, tO, u, Lm)` / `terms(a, ctx)`
（未知量なら `unknown: true` と `unit`）を実装した項目を足す。合計・未知量の解・式の行・
アニメーションはこのインターフェースしか見ていない。あとは
- `figure.js` の `render()` の荷重ループ（`isForce`/`isMoment`/`isDist` の分岐）に描き方と掴み代、
  `_layout()` に大きさ、`_dimRows()` に寸法線
- `app.js` の `TYPE_KEY`/`KEY_TYPE`（URL の略号）・`buildHash()`/`applyHash()`・`ROW_FIELDS`（入力欄）・
  `captionOf()`、`index.html` の `.add-btn`
...
- 図中の記号は**背景を塗らず、図の地の色（`--halo`）で縁取る**（`text-shadow` を 8 方向）。
  白い箱で塗ると線を切ってしまうため（2026-09-25、ユーザーの指示）
- 図中の分数は `\frac` → `\dfrac` に置き換えて描く（インラインの textstyle だと小さくて読めない。
  `beam/app.js` の `useDisplayFrac()` と同じ。2026-09-25、ユーザーの指示）
- 反力が O にあると O の真上が混むので、`M_O=0`／`M_O=?` のラベルは O の円弧の右上に置いている
...
- URL: `#m=pin|free&n=L|x&L=<mm>&o=<t>&a=<荷重>;...&e=<各荷重の円弧>&v=<図に数値>`。
  荷重は `p,t,P,dir`（集中荷重）／`r,t,dir`（未知反力）／`c,t,C`（集中モーメント、反時計まわり正）／
  `m,t,±1`（反力モーメントの仮定の向き）
- デバッグ用に `window.__moment = {state, update, fig, res}`

### `sfd-bmd/`（SFD・BMD。2026-09-25 実装、equilibrium/ を流用）

梁（曲げ）の `beam/` と機能は重なるが、「仮想断面を左から右へ動かし、V・M が変わっていく様子を
見る」ための専用ページ（ユーザーの要望）。Pyodide は使わず、計算は全部 JS。

**`equilibrium/` のファイルを直接 import / link している**（`../equilibrium/model.js` の `analyze`・`sumLines`・
`posTex`・`DIST_SHAPES` など、`../equilibrium/style.css` のテーマ・カード・フォーム・図の土台）。
単元をまたぐ依存なので、`equilibrium/` の関数の引数や CSS のクラス名を変えるときはこちらも確認すること。
（`common/` に切り出すのは 3 単元目で重複が問題になってからでよい、という CLAUDE.md の方針に沿って、
コピーではなく参照にした）

- **反力**: 支点を未知反力に置き換えて `analyze(mode: 'free', tO: 0)` に渡す（ピン・ローラー → 上向きの R、
  固定 → R と反時計まわりの M_R）。荷重は鉛直だけなのでピンの水平反力は省略。`status` が
  `under`（不静定・支点不足）/`inconsistent`（不安定）のときはグラフを薄くしてメッセージを出す。
  式の欄の「つり合いの式」は `sumLines()` の出力をそのまま出している（`M_O` は左端まわり）
- **記号**: 集中モーメントは曲げモーメント M と衝突するので **Beam.py と同じ `M_0, M_1, …`**（0 から）。
  そのために `equilibrium/model.js` の `namesOf()` に「荷重に `name` があればそれを使う」口を足した
  （equilibrium/ 自身は `name` を持たないので挙動は変わらない）。反力は `R` / `R_1…`、`M_R` / `M_{R1}…`
- **符号（Beam.py と同じ）**: 左側の自由体の右端の断面で、V は下向き、M は反時計まわり（下側引張）が正。
  `V = Σ(断面より左の上向きの力)`、`M = Σ F_i (x − a_i) − Σ C_i`。グラフは V・M とも上が正
  （Beam.py の SFD/BMD と同じ）。断面の図には V・M を**正の向き**で描き、値の符号で向きを読ませる
- **ちょうど断面の位置にある集中荷重は含めない**（`t < x`）。グラフは境目で左右の極限を両方入れて、
  跳びを縦線で描く。両端の外は 0 とし、支点の反力で 0 から跳ぶところも縦線で見せる
- **式（`sectionFormulas()`）**: いまの区間での V・M を x の式で書く。区間の中央で「左にあるか」を判定する
  （断面がちょうど境目にあっても式の形が揺れない）。通り過ぎた分布荷重は合力（図心に作用）に置きかえ、
  途中の分布荷重は `w(x − a)`、三角形は `rac{w}{2L}(x − a)^2` のように書く（係数は `wOverL()`）
- **M の最大**は V = 0 になる点を二分法で詰めている（標本の間に最大がある等分布・三角形のため）
- **3 枚の SVG（梁・SFD・BMD）は viewBox の横幅と `X0`/`LPX` を共通**にして、幅 100% で縦に並べるだけで
  x の位置がそろうようにしてある。縦の範囲だけ図ごとに決める。グラフの文字は SVG の `<text>` なので、
  狭い画面では `fz`（`W / clientWidth` に比例）で大きくし、グラフも縦長（`H`）にしている
- 断面より右は紙色の半透明の矩形で覆って薄くする。HTML の KaTeX ラベルには効かないので、
  断面より右の荷重・支点のラベルには `faded` を付けて `opacity` を下げている
- アニメーションは 6 秒で左端→右端、1.2 秒止めて繰り返す。アニメーション中は URL を書き換えない
  （止めたときに書く）
- URL: `#L=<mm>&s=<支点>;...&a=<荷重>;...&x=<断面>&g=<薄い線>&v=<数値>`。支点は `p|r|f,t`、荷重は
  `p,t,P,±1`（上向き 1）／`c,t,C`（反時計まわり正）／`d,t1,t2,w,u|r|l,±1`
- **グラフ・状態表示は数式ベース**（2026-09-25、ユーザーの要望）。数値は「図に数値も出す」がオンのときだけ
  単位つきで `= 2.28 kN·m` と添える。そのため `symbolic.js` で反力を荷重の記号で解いている
  （例 `R_1 = rac{2}{3}P`、未知量が 2 つで一意に解ける静定梁のとき。ΣM は L で割り、反力モーメントは
  `m = M_R/L` を未知量にして係数を数にしている）。断面力は項 `c·S·L^p·x^q` の多項式で持ち、
  `(x − aL)^n` は二項展開する。並びは教科書に合わせて x の次数の低い順（`rac12 wL − wx`）
  - グラフの最大・最小は、その位置が分数（分母 60 まで）なら式（`M_{\max} = rac18 wL^2`）、
    そうでなければ（三角形分布の最大など、位置が無理数）記号 `M_{\max}` だけ。跳びのある境目は
    左右の極限のうち数値が合う方の式を使う（`valueTexAt()`）
  - 断面の値の式は長くなるので、点のそばではなくグラフの空いている側の端（値が正なら下）に置く
  - ラベルの重なりは `DiagramPlot._declutter()` が DOM に入れてから実測し、最大・最小のラベルを
    点の反対側へずらす／それでも重なれば隠す（断面の値が優先）。**KaTeX のフォントが読み込まれると
    幅が変わるので、`document.fonts.ready` のあとに必ず描き直している**（しないと判定が古い幅のまま）
- **PC では梁の図・SFD・BMD を 1 画面に収める**（ユーザーの要望）。式のカードを図の右に置いて図の列を
  細くし（1100px 以上。`style.css`）、グラフの高さは `plotPx()` が「断面の位置のツールバー〜状態表示」が
  画面の高さに収まるように決める（`H` = viewBox の縦の半分）。操作の説明文は図の下に回した。
  測り直すのは設定の変更・画面サイズの変更・フォント読み込みのときだけ（`fitPlots()`。アニメーション中に
  毎フレーム測ると、状態表示の折り返しが変わってグラフの高さがちらつくため）。1280×720・1440×900・
  1920×1080 で収まることを確認済み。スマホ（600px 未満）では従来どおり `H = 150` で縦長
- 選んでいる支点・荷重は **Delete / Backspace キーで消せる**（入力欄に打っているときは除く）
- デバッグ用に `window.__sfd = {state, update, ctx, beam, sfd, bmd}`
...
- `equilibrium/`（力とモーメントのつり合い）… **実装済み**（集中荷重・未知反力・集中モーメント・反力モーメント・
  分布荷重（等分布・三角形）。上記`equilibrium/`の節）
